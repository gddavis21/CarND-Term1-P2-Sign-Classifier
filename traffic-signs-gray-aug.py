...
# def prep_images(images, offs_x, offs_y):
    # images_out = tf.cast(images, tf.float32)
    # images_out = tf.map_fn(lambda im: tf.image.crop_to_bounding_box(im, offs_y, offs_x, 24, 24), images_out)
    # images_out = tf.map_fn(lambda im: tf.image.per_image_standardization(im), images_out)
    # return images_out

# def prep_training_images(images):
    # images_out = tf.cast(images, tf.float32)
    # images_out = tf.map_fn(lambda im: tf.random_crop(im, [24,24,3]), images_out)
    # images_out = tf.map_fn(lambda im: tf.image.per_image_standardization(im), images_out)
    # return images_out.eval()
    
# def prep_test_images(images):
    # images_out = tf.cast(images, tf.float32)
    # images_out = tf.map_fn(lambda im: tf.image.crop_to_bounding_box(im, 4, 4, 24, 24), images_out)
    # images_out = tf.map_fn(lambda im: tf.image.per_image_standardization(im), images_out)
    # return images_out.eval()
    
# def prep_training_image(img):
    # y = random.randint(0,8)
    # x = random.randint(0,8)
    # img_out = grayscale(img)[y:y+24, x:x+24]
    # return (img_out.astype(np.float32) - 128.0)/128.0
    
# def prep_test_image(img):
    # img_out = grayscale(img)[4:28, 4:28]
    # return (img_out.astype(np.float32) - 128.0)/128.0

# def prep_training_images(images):
    # return np.array([prep_training_image(img) for img in images])
    
# def prep_test_images(images):
    # return np.array([prep_test_image(img) for img in images])
...
def inference(images, num_classes, weight_decay):    
    '''
    '''
    x_image = tf.reshape(images, [-1, 24, 24, 1])
    keep_prob = tf.placeholder(tf.float32)
    
    # Layer 1: Convolutional. Input = 24x24x1. Output = 24x24x32.
    conv_1 = my_conv2d(
        name='conv-1', 
        input=x_image, 
        weights_shape=[5,5,1,32], 
...
# def train_classifier(
    # x, y, keep_prob, training_op, accuracy_op, 
    ...
    # for i in range(num_epochs):
        ...
        # train_accuracy = evaluate_classifier(
            # x, y, keep_prob, accuracy_op, 
            # X_train, y_train, batch_size)
            
        # valid_accuracy = evaluate_classifier(
            # x, y, keep_prob, accuracy_op, 
